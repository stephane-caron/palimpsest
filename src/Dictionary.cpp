...
void Dictionary::update(const Dictionary &other) {
  ...
  if (this->is_value()) {
    *this = Dictionary();  // Clear value and become empty map
  }

  // At this point both are maps, we update recursively
  assert(this->is_map() && other.is_map());
  ...
  for (const auto &key_child : other.map_) {
    ...
    auto it = map_.find(key);
    if (it == map_.end()) {
      // Key doesn't exist in this dictionary, create new entry
      map_[key] = std::make_unique<Dictionary>();
      map_[key]->update(other_child);
    } else {
      // Key exists, replace it entirely
